...
install_frigate() {
    msg_info "🎥 Installation Frigate..."

    # Création des répertoires
    mkdir -p /opt/frigate /config /media/frigate
    mkdir -p /media/frigate/clips /media/frigate/recordings /media/frigate/exports

    # Installation Frigate via pip
    if python3 -m pip install $PIP_OPTS frigate >/dev/null 2>&1; then
        msg_ok "✅ Frigate installé via pip"
    else
        msg_warn "Installation pip échouée, tentative depuis le code source..."

        cd /opt
        if curl -L https://github.com/blakeblackshear/frigate/archive/v0.14.1.tar.gz -o frigate.tar.gz; then
            tar -xzf frigate.tar.gz
            mv frigate-0.14.1 frigate
            rm frigate.tar.gz
            cd frigate

            if python3 -m pip install $PIP_OPTS -e . >/dev/null 2>&1; then
                msg_ok "✅ Frigate installé depuis le code source"
            else
                msg_error "Échec installation Frigate"
                exit 1
            fi
        else
            msg_error "Impossible de télécharger Frigate"
            exit 1
        fi
    fi
}
...
configure_frigate() {
    msg_info "📝 Configuration Frigate..."

    # Configuration de base
    cat > /config/config.yml << 'EOF'
# Configuration Frigate de base
mqtt:
  enabled: false
...
# Détecteur CPU (pas besoin de GPU)
detectors:
  cpu1:
    type: cpu
    num_threads: 3
...
main() {
    ...
    if pct exec $CTID -- /root/frigate-install.sh; then
        ...
        echo ""
        echo -e "${GREEN}📱 Accès Frigate: http://$IP:5000${NC}"
